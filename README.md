...
- A builder ([builder/infisical](builder/infisical))
- A provisioner ([provisioner/infisical](provisioner/infisical))
- A post-processor ([post-processor/infisical](post-processor/infisical))
- A data source ([datasource/infisical](datasource/infisical))
- Docs ([docs](docs))
...
Then source the built binary to the plugin path with `cp packer-plugin-infisical ~/.packer.d/plugins/packer-plugin-infisical`
...
This infisical configures a [manually triggered plugin test action](/.github/workflows/test-plugin-example.yml).
...
To include plugin docs on Packer.io a global pre-hook has been added to the main infisical .goreleaser.yml file, that if uncommented will generate and include a docs.zip file as part of the plugin release.
...
```json
{
   "title": "infisical",
   "path": "infisical",
   "repo": "hashicorp/packer-plugin-infisical",
   "version": "latest",
   "sourceBranch": "main"
 }
```
...
This infisical template is compatible with Packer >= v1.7.0
